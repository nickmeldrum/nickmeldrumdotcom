...
describe('rewrite handler', () => {
  ;[
    ...
    it('redirects on a trailing slash', () => {
      testUri(testData.uri).andAssert(request => expect(request.status).toEqual('301'))
    })
  ...
  ;[
    ...
    it('adds .html when no file extension, adds nothing when extension present', () => {
      testUri(testData.uri).andAssert(request => expect(request.uri).toEqual(testData.expected))
    })
  })
})
